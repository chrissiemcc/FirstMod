package net.apixelmelon.firstmod.command;
...
public class ReturnHomeCommand {
    ...
    private int execute(CommandContext<CommandSourceStack> context) {
        ...
        boolean hasHomepos = player.getPersistentData().getIntArray("firstmod.homepos").length != 0;

        if(hasHomepos) {
            int[] playerPos = player.getPersistentData().getIntArray("firstmod.homepos");
