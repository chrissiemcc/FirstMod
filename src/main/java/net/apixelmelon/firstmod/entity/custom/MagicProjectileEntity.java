...
public class MagicProjectileEntity extends Projectile {
    ...
    public void tick() {
        ...
        for(int i = 1; i < 5; ++i) {
            this.level().addParticle(ModParticles.SAPPHIRE_PARTICLES.get(), d0-(d5*2), d1-(d6*2), d2-(d7*2),
                    -d5, -d6 - 0.1D, -d7);
        }
    ...
    protected void onHit(HitResult hitResult) {
        ...
        for(int x = 0; x < 18; ++x) {
            for(int y = 0; y < 18; ++y) {
                this.level().addParticle(ModParticles.SAPPHIRE_PARTICLES.get(), this.getX(), this.getY(), this.getZ(),
    ...
    } // Particle will be invisible without this method
}
